...
//A játék felállítása
auto setUpGame(){
    //Eltároljuk a játékost egy shared ptr-ben.
    auto player = std::make_shared<Warrior>(10, 10, 15);
    //Majd az ellenfelet is
    ...
    //A játékos arzenáljához adjuk a használható ability-ket.
    player->addAbility(attack);
    player->addAbility(fireslash);
    player->addAbility(heal);

    //Az ellenfél csak a kocka dobásával tud támadni
    enemy->addAbility(attack);

    //A Scneraiokat elkészítjük.
    ...
    std::string scenario_village_massacre_message = "Looking around, you see piles of dead bodies, houses burnt to the ground. What a terrible tragedy. You also see a broadsword. You pick it up.";
    ...
    //Majd feltöltjük a lehetséges szavakhoz tartozó következő Scenariokat
